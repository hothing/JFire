...
public class WebShopConfig
{
	...
	public WebShopConfig()
	{
		...
		if(organisationId == null)
			throw new IllegalStateException("Property "+PROPERTY_ORGANISATION_ID+" not set");
		userId = config.getProperty(PROPERTY_USER_ID);
		if(userId == null)
			throw new IllegalStateException("Property "+PROPERTY_USER_ID+" not set");
		workstationId = config.getProperty(PROPERTY_WORKSTATION_ID); // is optional - for now - later this will be required for the web-shop. Marco.
		password = config.getProperty(PROPERTY_PASSWORD);
		if(password == null)
			throw new IllegalStateException("Property "+PROPERTY_PASSWORD+" not set");
		currencyId = config.getProperty(PROPERTY_CURRENCY_ID);
		if(currencyId == null)
			throw new IllegalStateException("Property "+PROPERTY_CURRENCY_ID+" not set");
		providerUrl = config.getProperty(PROPERTY_PROVIDER_URL);
		if(providerUrl == null)
			throw new IllegalStateException("Property "+PROPERTY_PROVIDER_URL+" not set");
		initialContextFactory = config.getProperty(PROPERTY_INITIAL_CONTEXT_FACTORY);
		if(initialContextFactory == null)
			throw new IllegalStateException("Property "+PROPERTY_INITIAL_CONTEXT_FACTORY+" not set");
	}
