...
public class NanoHTTPD
{
	...
	public class Response
	{
		...
		public Response( String status, String mimeType, String txt )
		{
			this.status = status;
			this.mimeType = mimeType;
			this.data = new ByteArrayInputStream(txt.getBytes(charset));
		}
	...
	{
		this(port, address, Charset.defaultCharset().name());
	}
	 ...
	 * Throws an IOException if the socket is already in use
	 */
	public NanoHTTPD(int port, InetAddress address, String charset) throws IOException
	{
		this.myTcpPort = port;
...
//				StringTokenizer st = new StringTokenizer( in.readLine());
				StringTokenizer st = new StringTokenizer(readLine(isr));
				if ( !st.hasMoreTokens())
...
//							read = in.read(buf);
		...
		private String decodePercent( String str )
		{
			try {
				...
				// should never happen as we have the charset instance
...
//			{
				...
				if ( mime != null ) {
					String extendedMime = mime;
					if(!extendedMime.contains("charset="))
						extendedMime += "; charset="+charset.name();
					pw.print("Content-Type: " + extendedMime + "\r\n");
				}

				if ( header == null || header.getProperty( "Date" ) == null )
					pw.print( "Date: " + formatHeaderDate(new Date()) + "\r\n");
...
//				newUri += URLEncoder.encode( tok );
				...
				try { newUri += URLEncoder.encode( tok, charset.name() ); } catch ( UnsupportedEncodingException uee ) {}
			}
		}
		return newUri;
	}
							   ...
							   boolean allowDirectoryListing )
	{
		...
		if ( f.isDirectory())
		{
			...
			{
				uri += "/";
				Response r = new Response( HTTP_MOVED_PERMANENTLY, MIME_HTML,
	...
	protected String getMimeTypeByName(String path)
	{
		String mime = null;
		int dot = path.lastIndexOf( '.' );
		if ( dot >= 0 && dot+1 < path.length())
			mime = (String)theMimeTypes.get( path.substring( dot + 1 ).toLowerCase());
		if ( mime == null )
	...
	protected static String formatHeaderDate(Date date)
	{
