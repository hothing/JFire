...
extends LSDViewPart
{
	...
	/**
	 * Creates an IDoubleClickListener that defines the behaviour of this View with respect to whatever Perspective.
	 * Override this method for preparing other domain-specific IDoubleClickListener.
	 */
		...
		return new IDoubleClickListener() {
			...
			public void doubleClick(DoubleClickEvent event) {
				...
				if (object != null && object instanceof PersonRelation) {
					final PropertySetID personID = treeNode.getPropertySetID(); // getPropertySetID(objectID, object);
					if (personID != null) {
				...
				// Handles Issue stuffs ----------------------------------------------------------------------------->>
					...
					handleIssueRelatedNode(treeNode);
	...
	/**
	 * Modularised (since 2010.03.01): Handles behaviour of the tree related to Issues.
	 * @return true if an Issue-related object has been handled through the {@link IssueEditorInput}.
	 */
	protected boolean handleIssueRelatedNode(PersonRelationTreeNode node) {
		Object object = node.getJdoObject();
	...
	/**
	 * @return true if the given node contains an Issue-related object (or objectID).
	 */
	public boolean isNodeOfIssueRelatedType(PersonRelationTreeNode node) {
		Object jdoObject = node.getJdoObject();
		ObjectID jdoObjectID = node.getJdoObjectID();

		return jdoObject instanceof IssueLink || jdoObject instanceof IssueComment || jdoObjectID instanceof IssueDescriptionID;
	}

	@Override
	public void createPartContents(Composite parent) {
		personRelationTree = createPersonRelationTree(parent); // new PersonRelationTree(parent);
			...
			public void selectionChanged(SelectionChangedEvent event)
			{
				...
				if (selectedElement instanceof PersonRelationTreeNode) {
					...
					if (node != null) {
						...
						PropertySetID personID = node.getPropertySetID(); // getPropertySetID(jdoObjectID, jdoObject);
						if (personID != null)
