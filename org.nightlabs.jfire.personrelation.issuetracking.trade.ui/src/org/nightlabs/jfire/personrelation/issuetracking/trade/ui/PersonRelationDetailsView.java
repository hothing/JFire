...
//		editorSection.setText("RESERVED the configurable quick-details customer-view/section");
		...
		personSelectionListener = new NotificationAdapterSWTThreadAsync() {
			...
			public void notify(NotificationEvent notificationEvent) {
				if (personRelationDetailsComposite == null || personRelationDetailsComposite.isDisposed())
					return;

				// See notes on CRM-perspective's listener management.
				// --> If the notificationEvent contains the LegalEntity class in its carrier,
				//     we do not process it if the event contains more than one subject.
				List<Class<?>> subjectClassesToClear = notificationEvent.getSubjectClassesToClear();
				Set<?> subjects = notificationEvent.getSubjects();
				if (subjectClassesToClear != null && !subjectClassesToClear.isEmpty()) {
					if (subjectClassesToClear.get(0).equals(LegalEntity.class)) {
